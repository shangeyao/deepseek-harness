...
/**
 * Response headers that describe the upstream wire encoding. Node fetch
 * decompresses gzip/br bodies before proxyHttp reads them, so forwarding these
 * headers would make browsers attempt a second decode and break HTML/JS.
 */
const UPSTREAM_BODY_DESCRIPTOR = new Set(['content-encoding', 'content-length'])

/** Build the client-facing response header map from an upstream fetch Response. */
function proxiedResponseHeaders(upstream: Response): Record<string, string | string[]> {
  const headers: Record<string, string | string[]> = {}
  for (const [key, value] of upstream.headers.entries()) {
    const lower = key.toLowerCase()
    if (HOP_BY_HOP.has(lower) || UPSTREAM_BODY_DESCRIPTOR.has(lower)) continue
    headers[lower] = value
  }
...
export function buildUpstreamHeaders(
  ...
  // Node fetch auto-decompresses; ask the upstream for identity bytes.
...
export async function proxyHttp(
  ...
  res.writeHead(upstream.status, proxiedResponseHeaders(upstream))
...
function swallowSocketError(socket: Duplex): void {
  socket.on('error', () => {
    socket.destroy()
  })
}
...
export function proxyWebSocket(
  ...
  upstream.on('upgrade', (upstreamRes, upstreamSocket, upstreamHead) => {
    ...
    swallowSocketError(upstreamSocket)
